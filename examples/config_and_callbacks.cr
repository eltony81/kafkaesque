...
puts "[CONFIG] Loading configurations from #{config_file}..."
...
producer.on_deliver do |topic, partition, offset, exception|
  if exception
    puts "[ERROR] [Producer Callback] Delivery failed: #{exception.message}"
  else
    puts "[SUCCESS] [Producer Callback] Confirmed delivery on #{topic}:#{partition} at offset #{offset}"
  end
end

# Callback B: Reports client performance stats periodically
producer.on_stats do |stats_json|
  puts "[STATS] [Producer Stats] #{stats_json}"
end
...
consumer.on_partitions_assigned do |partitions|
  puts "[ASSIGNED] [Consumer Callback] Assigned partitions: #{partitions}"
end

# Callback D: Triggered when partition ownership is revoked
consumer.on_partitions_revoked do |partitions|
  puts "[REVOKED] [Consumer Callback] Revoked partitions: #{partitions}"
end
...
begin
  puts "[START] Producing test messages..."
...
begin
  puts "[START] Consumer listening for events..."
  # Streaming loop
