module Kafkaesque
  class Client
    ...
    def add_offsets_to_txn(transactional_id : String, producer_id : Int64, producer_epoch : Int16, group_id : String) : Protocol::AddOffsetsToTxnResponse
      conn = @connection || raise "Client is not connected."
      req = Protocol::AddOffsetsToTxnRequest.new(transactional_id, producer_id, producer_epoch, group_id)

      req_io = IO::Memory.new
      req_enc = Protocol::Encoder.new(req_io)
      req_header = Protocol::RequestHeader.new(
        api_key: Protocol::AddOffsetsToTxnRequest::API_KEY,
        api_version: Protocol::AddOffsetsToTxnRequest::API_VERSION,
        correlation_id: next_correlation_id,
        client_id: @client_id,
        flexible: false
      )
      ...
      response_dec = Protocol::Decoder.new(response_io)
      Protocol::ResponseHeader.deserialize(response_dec, flexible: false)
      Protocol::AddOffsetsToTxnResponse.deserialize(response_dec)
    end
