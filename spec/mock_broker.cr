    ...
    private def handle_client(socket)
      loop do
        ...
        begin
          ...
          # Skip flexible tag header for requests in API version >= some flexible threshold if needed
          # But for our mock metadata and standard endpoints, we match what client sends.
          ...
          # Form full response:
          # Total Response size = 4 (correlation_id) + response_body_io size
          # In flexible/non-flexible headers: ResponseHeader has correlation_id (4 bytes)
